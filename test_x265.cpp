...
void InitEncodeParam(x265_param &param, int initial_bitrate, int frame_rate, int width, int height, double vbv_buffer_size, int qp_step) {
    x265_param_default_preset(&param, PRESET, "zerolatency");
...
void UpdateBitrateConfig(x265_encoder *encoder, x265_param &param, int bitrate, double vbv_buffer_size) {
    ...
    param.rc.vbvMaxBitrate = bitrate; // * 10;
    param.rc.vbvBufferSize = bitrate * vbv_buffer_size;

    int ret = x265_encoder_reconfig(encoder, &param);
...
void EncodeAndGenerateStatistics(const string &video_name, const string &bitrate_config_filename, const string &output_dir, int initial_bitrate, int frame_rate, int width, int height, double vbv_buffer_size, int response_time, const string &suffix, int qp_step, double dropped_vbv_buffer_size, int reduce_number) {
    ...
    if (!pic || !pic_out) {
        cout << "x265_picture_alloc failed" << endl;
        return;
    }
    x265_picture_init(&param, pic);
    cout << "pic.width: " << pic->width << " pic.height: " << pic->height << endl;

    encoder = x265_encoder_open(&param);
    if( !encoder ) return;

    int luma_size = param.sourceWidth * param.sourceHeight;
    ...
    for(;; i_frame++) {
        ...
        pic->planes[0] = buff;
        pic->planes[1] = buff + luma_size;
        pic->planes[2] = buff + luma_size * 5 / 4;
        pic->stride[0] = param.sourceWidth;
        pic->stride[1] = param.sourceWidth / 2;
        pic->stride[2] = param.sourceWidth / 2;
        if (fread(pic->planes[0], 1, luma_size, input_yuv_file) != (unsigned)luma_size) break;
        if (fread(pic->planes[1], 1, chroma_size, input_yuv_file) != (unsigned)chroma_size) break;
        if (fread(pic->planes[2], 1, chroma_size, input_yuv_file) != (unsigned)chroma_size) break;
        ...
        x265_encoder_encode(encoder, &nal, &i_nal, pic, NULL);
        i_frame_size = 0;
        for (int i = 0; i < i_nal; i++) {
    ...
    //         return;
    //     }
    //     cout << "Write delayed frame: " << i_frame + 1 << " nal type: " << nal->i_type << " size: " << i_frame_size << endl;
    // }
...
int main(int argc, char* argv[]) {
    ...
    int response_time = 1;//std::strtod(argv[5], &end);
